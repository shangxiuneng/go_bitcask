...
func TestHash_Delete(t *testing.T) {
	...
	// 删除一个不存在的元素
	oldValue, err := hashIndex.Delete([]byte("key"))
	assert.Nil(t, err)
	assert.Nil(t, oldValue)

	// put一个key再删除
	oldValue, err = hashIndex.Put([]byte("key"), &data.RecordPos{FileID: 1, Offset: 10})
	assert.Nil(t, err)
	assert.Nil(t, oldValue)
	...
	oldValue, err = hashIndex.Delete([]byte("key"))
	assert.Nil(t, err)
	assert.Equal(t, &data.RecordPos{FileID: 1, Offset: 10}, oldValue)
	t.Logf("fileID = %v,offSet = %v", oldValue.FileID, oldValue.Offset)
	pos, err = hashIndex.Get([]byte("key"))
	assert.NotNil(t, err)
}
